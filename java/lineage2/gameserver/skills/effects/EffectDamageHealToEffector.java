...
public class EffectDamageHealToEffector extends EffectDamOverTime
{
	...
	private class DamageListener implements OnCurrentHpDamageListener
	{
		...
		{
			if ((_effector.getPlayer() != null) && (_effector.getPlayer().getSummonList() != null))
			{
				for (Summon summon : _effector.getPlayer().getSummonList())
				{
					summon.setCurrentHp(summon.getCurrentHp() + ((damage * _effected.calcStat(Stats.DAMAGE_HEAL_TO_EFFECTOR, 0.0D)) / 100.0D), false);
				}
			}
			_effector.setCurrentHp(_effector.getCurrentHp() + ((damage * _effected.calcStat(Stats.DAMAGE_HEAL_TO_EFFECTOR, 0.0D)) / 100.0D), false);
		}
	}
