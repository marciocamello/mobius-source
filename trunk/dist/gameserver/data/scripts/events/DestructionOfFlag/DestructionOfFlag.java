...
public class DestructionOfFlag extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener
{
	 ...
	 * Field _log.
	 */
	static Logger _log = LoggerFactory.getLogger(DestructionOfFlag.class.getName());
	/**
	...
	public static void go(String s)
	{
		...
		{
			Announcements.getInstance().announceToAll("DestructionOfFlag: reg completed, was not attained the minimum number of players.");
	...
	public static void endBattle(int win)
	{
		...
		if (win != 0)
		{
			if (win == 1)
			{
				Announcements.getInstance().announceToAll("The White Team Ton!");
