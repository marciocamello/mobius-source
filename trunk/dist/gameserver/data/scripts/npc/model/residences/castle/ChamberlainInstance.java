		...
		if ((castle != null) && (castle.getId() > 0))
		{
			if (player.getClan() != null)
			{
				...
				else if (castle.getOwnerId() == player.getClanId())
				{
					...
					if (isHaveRigths(player, Clan.CP_CS_ENTRY_EXIT) || isHaveRigths(player, Clan.CP_CS_MANOR_ADMIN) || isHaveRigths(player, Clan.CP_CS_MANAGE_SIEGE) || isHaveRigths(player, Clan.CP_CS_USE_FUNCTIONS) || isHaveRigths(player, Clan.CP_CS_DISMISS) || isHaveRigths(player, Clan.CP_CS_TAXES) || isHaveRigths(player, Clan.CP_CS_MERCENARIES) || isHaveRigths(player, Clan.CP_CS_SET_FUNCTIONS))
					{
						return COND_OWNER;
					}
				}
