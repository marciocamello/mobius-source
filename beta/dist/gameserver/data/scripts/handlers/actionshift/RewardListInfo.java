...
public final class RewardListInfo
{
	...
	static void showInfo(Player player, NpcInstance npc)
	{
		...
		{
			htmlMessage.setFile("actionshift/rewardlist_to_weak.htm");
		...
		{
			htmlMessage.setFile("actionshift/rewardlist_empty.htm");
			player.sendPacket(htmlMessage);
			return;
		}
		
		htmlMessage.setFile("actionshift/rewardlist_info.htm");
	...
	private static void ratedGroupedRewardList(StringBuilder tmp, NpcInstance npc, RewardList list, Player player, double mod)
	{
		...
		for (RewardGroup g : list)
		{
			...
			for (RewardData d : items)
			{
				...
				tmp.append("<tr><td width=32><img src=").append(icon).append(" width=32 height=32></td><td width=238>").append(HtmlUtils.htmlItemName(d.getId())).append("<br1>");
