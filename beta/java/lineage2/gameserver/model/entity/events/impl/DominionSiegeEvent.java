...
public class DominionSiegeEvent extends SiegeEvent<Dominion, SiegeClanObject>
{
	...
	public void startEvent()
	{
		...
		{
			// owner current territory, attaker, in all other
			d.addObject(ATTACKERS, ownerClan);
			
			// all mercenaries are attaking other territories
	...
	public void questUpdate(Player player, boolean start)
	{
		if (start)
		{
			...
			Quest protectCatapultQuest = QuestManager.getQuest("Q00729_ProtectTheTerritoryCatapult");
	...
	public boolean canRessurect(Player resurrectPlayer, Creature target, boolean force)
	{
		...
		// if target is a siege zone - res enabled
		if (!targetInZone)
		{
			return false;
		}
		...
		// if target is not our siege (or even no siege) - res disabled
		...
		// if there is no flag - res disabled
	...
	public int[] calculateReward(Player player)
	{
		...
		int[] out = new int[3];
		// static (start, stop, quests, etc.)
		out[0] += rewards[STATIC_BADGES];
		// if more than 14 online Revard (70 min in the area) is 7 max
		...
		// TODO [VISTALL] incorrectly, fame and given below, there is no issue of Adena
		if (out[0] > 90)
		{
			out[0] = 90; // badges
