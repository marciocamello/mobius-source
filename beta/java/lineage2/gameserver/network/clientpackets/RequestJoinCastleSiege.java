...
public class RequestJoinCastleSiege extends L2GameClientPacket
{
	...
	private static void registerAtCastle(Player player, Castle castle, boolean attacker, boolean join)
	{
		...
		if (join)
		{
			...
			if (attacker)
			{
				...
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CASTLE_OWNING_CLANS_ARE_AUTOMATICALLY_REGISTERED_ON_THE_DEFENDING_SIDE));
					return;
				}
				...
				if (alliance != null)
				{
					for (Clan clan : alliance.getMembers())
					{
						...
						{
							player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_CANNOT_REGISTER_AS_AN_ATTACKER_BECAUSE_YOU_ARE_IN_AN_ALLIANCE_WITH_THE_CASTLE_OWNING_CLAN));
							return;
						}
					}
				}
				...
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.A_CLAN_THAT_OWNS_A_CASTLE_CANNOT_PARTICIPATE_IN_ANOTHER_SIEGE));
					return;
				}
				
				if (siegeClan != null)
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_ALREADY_REGISTERED_TO_THE_ATTACKER_SIDE_AND_MUST_CANCEL_YOUR_REGISTRATION_BEFORE_SUBMITTING_YOUR_REQUEST));
					return;
				}
				
				if (playerClan.getLevel() < 5)
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.ONLY_CLANS_OF_LEVEL_5_OR_ABOVE_MAY_REGISTER_FOR_A_CASTLE_SIEGE));
					return;
				}
				
				if (registeredCastle != null)
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_HAVE_ALREADY_REQUESTED_A_CASTLE_SIEGE));
					return;
				}
				
				if (siegeEvent.isRegistrationOver())
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_TOO_LATE_THE_REGISTRATION_PERIOD_IS_OVER));
					return;
				}
				
				if (castle.getSiegeDate().getTimeInMillis() == 0)
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.THIS_IS_NOT_THE_TIME_FOR_SIEGE_REGISTRATION_AND_SO_REGISTRATION_AND_CANCELLATION_CANNOT_BE_DONE));
					return;
				}
				...
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NO_MORE_REGISTRATIONS_MAY_BE_ACCEPTED_FOR_THE_ATTACKER_SIDE));
					return;
				}
				...
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.SIEGE_REGISTRATION_IS_NOT_POSSIBLE_DUE_TO_YOUR_CASTLE_CONTRACT));
					return;
				}
			...
			else
			{
				...
				if (siegeClan != null)
				{
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_HAVE_ALREADY_REGISTERED_TO_THE_DEFENDER_SIDE_AND_MUST_CANCEL_YOUR_REGISTRATION_BEFORE_SUBMITTING_YOUR_REQUEST));
					return;
				}
		...
		else
		{
			...
			if (siegeClan == null)
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_NOT_YET_REGISTERED_FOR_THE_CASTLE_SIEGE));
				return;
			}
	...
	private static void registerAtClanHall(Player player, ClanHall clanHall, boolean join)
	{
		...
		if (join)
		{
			...
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.A_CLAN_THAT_OWNS_A_CLAN_HALL_MAY_NOT_PARTICIPATE_IN_A_CLAN_HALL_SIEGE));
				return;
			}
			...
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.ONLY_CLANS_WHO_ARE_LEVEL_4_OR_ABOVE_CAN_REGISTER_FOR_BATTLE_AT_DEVASTATED_CASTLE_AND_FORTRESS_OF_THE_DEAD));
				return;
			}
