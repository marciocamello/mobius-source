		...
		else
		{
			...
			for (Object[] element : ITEMS)
			{
				...
				if (((item >= 6011) && (item <= 6031)) || ((item >= 6320) && (item <= 6321)))
				{
					...
					{
						amt += 1;
						html += "<tr><td height=45><img src=icon." + element[1] + " height=32 width=32></td><td width=180>" + element[2] + "</td><td><button value=X1 action=\"bypass -h Quest _373_SupplierOfReagents x_1_" + page[1] + "_" + str(item) + "\" width=40 height=15 fore=sek.cbui92><button value=X10 action=\"bypass -h Quest _373_SupplierOfReagents x_2_" + page[1] + "_" + str(item) + "\" width=40 height=15 fore=sek.cbui92></td></tr>";
					}
				}
			}
			if (amt == 0)
