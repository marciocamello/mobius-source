...
public class Q00255_Tutorial extends Quest implements ScriptFile, OnPlayerEnterListener
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if (event.startsWith("UC"))
		{
			...
			else if ((level == 20) && (player.getQuestState("Q10276_MutatedKaneusGludio") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 28) && (player.getQuestState("Q10277_MutatedKaneusDion") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 28) && (player.getQuestState("Q10278_MutatedKaneusHeine") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 28) && (player.getQuestState("Q10279_MutatedKaneusOren") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 28) && (player.getQuestState("Q10280_MutatedKaneusSchuttgart") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 28) && (player.getQuestState("Q10281_MutatedKaneusRune") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
			else if ((level == 79) && (player.getQuestState("Q00192_SevenSignsSeriesOfDoubt") == null))
			{
				st.showQuestionMark(36);
				st.playSound(SOUND_TUTORIAL);
			}
